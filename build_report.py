...
# ✨ 새롭게 추가된 대상들을 여기에 등록합니다. 
# 첫 번째 값은 scrape_amazon_beauty.py에서 생성한 시트 이름과 일치해야 합니다.
COUNTRIES = [
    ("US", "미국(전체)"),
    ("US_SkinCare", "미국(스킨케어)"),
    ("US_SunCare", "미국(선크림)"),
...
def _gray_border():
    return Border(left=GRAY_BORDER, right=GRAY_BORDER, top=GRAY_BORDER, bottom=GRAY_BORDER)
...
def _kcell(ws, row, col, value=None, fill=None, bold=False, font_color="2D2D2D", size=9, h_align="left", wrap=False):
    ...
    if fill: c.fill = fill
    c.font = Font(bold=bold, color=font_color, size=size)
    c.alignment = Alignment(horizontal=h_align, vertical="center", wrap_text=wrap, shrink_to_fit=False)
...
def write_korean_summary(wb, country_data, brand_map):
    ...
    for ci, (en, ko) in enumerate(COUNTRIES):
        base = ci * STRIDE + 1
        ws.merge_cells(start_row=1, start_column=base, end_row=1, end_column=base + COLS_PER - 1)
        _kcell(ws, 1, base, "Best Sellers in Beauty", fill=GRAY_TITLE, bold=True, font_color="FFFFFF", h_align="center", size=9)

    ws.row_dimensions[2].height = 18
    for ci, (en, ko) in enumerate(COUNTRIES):
        base = ci * STRIDE + 1
        ws.merge_cells(start_row=2, start_column=base, end_row=2, end_column=base + COLS_PER - 1)
        _kcell(ws, 2, base, ko, fill=GRAY_COUNTRY, bold=True, font_color="FFFFFF", h_align="center", size=10)
    ...
    for ci in range(len(COUNTRIES)):
        ...
        for j, lbl in enumerate(col_labels):
            _kcell(ws, 3, base + j, lbl, fill=GRAY_HEADER, bold=True, h_align="center", size=9)
    ...
    print(f"  [한국 정리] 시트 완료 — " + ", ".join(f"{ko}:{n}개" for (_, ko), n in zip(COUNTRIES, counts)))

# ─── Main ─────────────────────────────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--scraped", default=f"amazon_beauty_bestsellers_{TODAY}.xlsx", help="스크래핑된 xlsx 파일 경로")
    parser.add_argument("--output", default=f"amazon_beauty_report_{TODAY}.xlsx", help="출력 파일 경로")
